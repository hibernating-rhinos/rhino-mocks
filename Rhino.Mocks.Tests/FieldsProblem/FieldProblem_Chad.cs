...
namespace Rhino.Mocks.Tests.FieldsProblem
{
	public class FieldProblem_Chad
	{
		...
		{
			TestClass test = MockRepository.GenerateStub<TestClass>();
			test.Stub(x => x.ReadOnly).Return("foo");
...
You can use the property directly to achieve the same result: mockObject.SomeProperty = 42;";
			...
			var ex = Assert.Throws<InvalidOperationException>(() => test.Stub(x => x.ReadWrite).PropertyBehavior());
